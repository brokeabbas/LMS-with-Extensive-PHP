...
if (isset($_GET['student_id'])) {
    ...
    try {
        // Update query to set is_active to 1 in 'student_info'
        $query = "UPDATE student_info SET is_active = 1 WHERE id = ?";
        ...
        $userInfoConn->commit();
        echo "<script>alert('Student account has been enabled successfully.'); window.location.href='manage-existing-students.php';</script>";
    } catch (Exception $e) {
        // Rollback the transaction on error
        $userInfoConn->rollback();
        echo "<script>alert('Failed to enable the student account: " . $e->getMessage() . "'); window.location.href='manage-existing-students.php';</script>";
    }
}
